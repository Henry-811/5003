...
log_file_rf <- "random_forest_optimization.log"
...
log_file_glmnet <- "glmnet_optimization.log"
...
log_file_dt <- "decision_tree_optimization.log"
...
# Define custom Macro F1 evaluation metric for XGBoost
# XGBoost eval functions: f(preds, dtrain) -> list(metric_name, value)
# Higher Macro F1 = better, so we return negative value for minimization
macro_f1_eval <- function(preds, dtrain) {
  labels <- getinfo(dtrain, "label")

  # Reshape predictions to class probabilities
  num_class <- length(preds) / length(labels)
  preds_matrix <- matrix(preds, ncol = num_class, byrow = TRUE)
  pred_class <- max.col(preds_matrix) - 1

  # Calculate per-class F1
  all_classes <- 0:(num_class - 1)
  f1_per_class <- sapply(all_classes, function(l) {
    tp <- sum(labels == l & pred_class == l)
    fp <- sum(labels != l & pred_class == l)
    fn <- sum(labels == l & pred_class != l)
  ...
  # Return negative because XGBoost minimizes custom metrics
  return(list(metric = "macro_f1", value = -macro_f1))
}

# Create optimization log
log_file_xgb <- "xgboost_optimization.log"
...
xgb_cv_bayes <- function(max_depth, eta, subsample, colsample_bytree, min_child_weight, gamma) {
  ...
  cv_results <- foreach(fold_idx = 1:5, .packages = c("xgboost"),
                        .combine = rbind, .export = c("folds", "X_train", "y_train", "sample_weights", "macro_f1_eval", "params")) %dopar% {
